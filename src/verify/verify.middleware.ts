...
export class VerifyMiddleware implements NestMiddleware {
  use(req: any, res: any, next: () => void) {
    try {
        ...
        const decoded = await jwt.verify(token, process.env.TOKEN_SECRET as Secret) as JwtPayload;
        ...
        const foundDevice = await foundUser.devices.find((device: IDevice) => device.ip === req.socket.remoteAddress);
